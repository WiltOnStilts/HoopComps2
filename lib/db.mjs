...
import { normalizePhone, normalizeUsername, looksLikePhone, displayNameKey } from "./user-lookup.mjs";
...
export async function assertDisplayNameAvailable(userId, displayName) {
  const key = displayNameKey(displayName);
  if (!key) return;

  if (backend === "postgres") {
    await pgStore.pgAssertDisplayNameAvailable(userId, displayName);
    return;
  }
  if (backend === "file") {
    fileStore.fileAssertDisplayNameAvailable(userId, displayName);
    return;
  }
    ...
    ? sqlite()
        .prepare(
          `SELECT id FROM users
           WHERE id <> ?
             AND lower(trim(replace(display_name, '  ', ' '))) = ?
           LIMIT 1`
        )
        .all(userId, key)
    : sqlite()
        .prepare(
          `SELECT id FROM users
           WHERE lower(trim(replace(display_name, '  ', ' '))) = ?
           LIMIT 1`
        )
        .all(key);
...
export async function searchUsersByFriendQuery(query, { excludeUserId, limit = 12 } = {}) {
  const { parseFriendSearchQuery, userMatchesFriendSearch } = await import("./user-lookup.mjs");
  const parsed = parseFriendSearchQuery(query);
  if (!parsed) return [];

  if (backend === "postgres") {
    return pgStore.pgSearchUsersByFriendQuery(query, { excludeUserId, limit });
  }
  if (backend === "file") {
    return fileStore.fileSearchUsersByFriendQuery(query, { excludeUserId, limit });
  }

  const exclude = excludeUserId || "__none__";
  const like = `${parsed.term}%`;
  const phoneLike = parsed.mode === "phone" ? `${parsed.term}%` : like;
  const rows = sqlite()
    .prepare(
      `SELECT id, email, display_name, username, phone FROM users
       WHERE id <> ?
         AND (
           ...
           OR lower(replace(display_name, ' ', '')) LIKE ?
           OR lower(substr(email, 1, CASE WHEN instr(email, '@') > 0 THEN instr(email, '@') - 1 ELSE length(email) END)) LIKE ?
           OR phone LIKE ?
         )
       ORDER BY display_name ASC
       LIMIT ?`
    )
    .all(exclude, like, like, like, like, phoneLike, limit);

  return rows.filter((row) => userMatchesFriendSearch(row, parsed));
}

function assertUniqueContactFieldsSqlite(userId, { displayName, username, phone }) {
  ...
  if (nameKey) {
    const clash = sqlite()
      ...
      .get(userId, nameKey);
    if (clash) throw new Error("Display name already taken");
  }
...
function updateUserContactFieldsSqlite(userId, { displayName, username, phone }) {
  assertUniqueContactFieldsSqlite(userId, { displayName, username, phone });
...
export async function createUser({ id, email, passwordHash, displayName }) {
  await assertDisplayNameAvailable(null, displayName || "Scout");
  if (backend === "postgres") {
