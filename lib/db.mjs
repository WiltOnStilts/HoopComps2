...
export function updatePasswordHash(userId, passwordHash) {
  if (useFileStore) {
    fileStore.fileUpdatePasswordHash(userId, passwordHash);
    return;
  }
...
export function saveUserState(userId, state, { publicLeaderboard } = {}) {
  ...
  if (name) {
    if (useFileStore) {
      ...
      notifySpotlightPool(userId, state);
      return;
    }
    sqlite()
...
function notifySpotlightPool(userId, state) {
  const user = findUserById(userId);
  persistSpotlightPool(userId, user?.email, state, findUserByEmail);
}
