...
function applyScanTracking(state) {
...
export async function getUserState(userId) {
  ...
  if (result?.state) result.state = applyScanTracking(result.state);
  return result;
}
  ...
  return sortLeaderboardEntries(
    rows
      .map((row) => {
        let state;
        try {
          state = applyScanTracking(JSON.parse(row.state_json));
        } catch {
          return null;
        }
