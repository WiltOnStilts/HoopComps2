...
export function purgeDailyCodData(data) {
  ...
  if (Array.isArray(data.codVotes)) {
    const votesBefore = data.codVotes.length;
    data.codVotes = data.codVotes.filter((v) => v.dayKey === dayKey);
    if (data.codVotes.length !== votesBefore) changed = true;
  } else {
    data.codVotes = [];
  }
