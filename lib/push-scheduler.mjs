...
async function runUserScheduledNotifications(now = new Date()) {
  ...
  for (const { userId, timezoneOffsetMinutes } of users) {
    try {
      ...
      const scoutedToday = hasScoutedToday(state, timezoneOffsetMinutes, now);
      const needsStreakReminder = streak > 0 && !scoutedToday;
      ...
      } else if (!scoutedToday) {
        const nudgeHour = STREAK_REMINDER_HOURS[0];
        if (hasReachedLocalHour(now, timezoneOffsetMinutes, nudgeHour)) {
      ...
      if (
        hasReachedLocalHour(now, timezoneOffsetMinutes, SCOUT_PROMPT_HOUR) &&
        !scoutedToday
      ) {
        const dedupeKey = scoutPromptDedupeKey(localDay);
        await sendPushToUser(userId, PUSH_TYPES.scout_prompt, dedupeKey, { dedupeKey });
