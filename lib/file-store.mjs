...
import { normalizePhone, normalizeUsername, parseFriendSearchQuery, userMatchesFriendSearch, displayNameKey } from "./user-lookup.mjs";
...
export function fileSearchUsersByFriendQuery(query, { excludeUserId, limit = 12 } = {}) {
  ...
  for (const [id, u] of Object.entries(store.users || {})) {
    ...
    const row = mapUser(id, u);
    if (userMatchesFriendSearch(row, parsed)) results.push(row);
...
export function fileAssertDisplayNameAvailable(userId, displayName) {
  const key = displayNameKey(displayName);
  if (!key) return;
  const store = readStore();
  for (const [id, u] of Object.entries(store.users || {})) {
    if (userId && id === userId) continue;
    if (displayNameKey(u.displayName) === key) {
...
export function fileCreateUser({ id, email, passwordHash, displayName }) {
  fileAssertDisplayNameAvailable(null, displayName || "Scout");
  const store = readStore();
...
export function fileSaveUserState(userId, state, { publicLeaderboard } = {}) {
  ...
  if (user) {
    ...
    if (name) {
      fileAssertDisplayNameAvailable(userId, name);
