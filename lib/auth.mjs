...
import { mergeScannedCards, mergeCollectionByFingerprint } from "./card-fingerprint.mjs";
...
export function mergeGuestIntoCloud(guestState, cloudState) {
  ...
  if (!guestState?.collection?.length) {
    const scannedCards = mergeScannedCards(cloudState?.scannedCards, guestState?.scannedCards);
    const scoutCount =
      Object.keys(scannedCards).length ||
      Math.max(cloudState?.scoutCount || 0, guestState?.scoutCount || 0);
    return {
      ...cloudState,
      ...guestState,
