...
import { findUserByEmail, findUserById, findUserByHandle, findUserByFriendLookup, searchUsersByFriendQuery, getUserState } from "./db.mjs";
import { uniqueScoutCount } from "./card-fingerprint.mjs";
import {
  moderateUserContent,
  COMMENT_BAN_DAYS,
  formatBanUntil,
} from "./content-moderation.mjs";
import { notifyFriendRequestAccepted } from "./push.mjs";
...
function publicSearchUser(user) {
  ...
  return {
    id: user.id,
    displayName: user.display_name || user.displayName || "Scout",
    username: user.username || null,
    hasPhone: Boolean(user.phone),
  };
}

async function pendingBetween(data, userA, userB) {
...
export async function searchFriendCandidates(viewerId, query) {
  const parsed = (await import("./user-lookup.mjs")).parseFriendSearchQuery(query);
  if (!parsed) {
    return { count: 0, results: [], query: String(query || "").trim(), ready: false };
  }

  const rawMatches = await searchUsersByFriendQuery(query, { excludeUserId: viewerId, limit: 15 });
  const data = await loadSocial();
  const friendIds = new Set();
  for (const f of data.friendships) {
    if (f.userA === viewerId) friendIds.add(f.userB);
    if (f.userB === viewerId) friendIds.add(f.userA);
  }

  const results = [];
  for (const row of rawMatches) {
    if (friendIds.has(row.id)) continue;
    const pending = await pendingBetween(data, viewerId, row.id);
    results.push({
      ...publicSearchUser(row),
      pending: Boolean(pending),
      incomingRequest: pending?.toUserId === viewerId,
    });
  }
...
export async function sendFriendRequest(fromUserId, identifier) {
  const lookup = typeof identifier === "object" ? null : identifier?.trim();
  const targetUserId = typeof identifier === "object" ? identifier?.targetUserId : null;

  let target = null;
  if (targetUserId) {
    target = await findUserById(targetUserId);
  } else {
    if (!lookup) throw new Error("Enter a phone number or username");
    target = await findUserByFriendLookup(lookup);
  }
  if (!target) throw new Error("No collector found with that phone or username");
...
export async function respondFriendRequest(userId, requestId, action) {
  ...
  if (action === "accept") {
    const accepter = await userSummary(userId);
    void notifyFriendRequestAccepted(req.fromUserId, {
      requestId,
      displayName: accepter?.displayName || "A collector",
    });
  }
...
export async function getFriendAccount(viewerId, targetUserId) {
  ...
  return {
    ...
    stats: {
      coins: state.coins || 0,
      streak: state.streak || 0,
      scoutCount: uniqueScoutCount(state),
...
function buildCodPoll(data, dayKey, viewerUserId = null) {
  const votes = (data.codVotes || []).filter((v) => v.dayKey === dayKey);
  const hold = votes.filter((v) => v.vote === "hold").length;
  const sell = votes.filter((v) => v.vote === "sell").length;
  const total = hold + sell;
  const userVote = viewerUserId
    ? votes.find((v) => v.userId === viewerUserId)?.vote || null
    : null;
  return {
    hold,
    sell,
    total,
    holdPercent: total ? Math.round((hold / total) * 100) : 0,
    sellPercent: total ? Math.round((sell / total) * 100) : 0,
    userVote,
  };
}
...
export async function listCodComments(viewerUserId = null) {
  ...
  return {
    dayKey,
    comments,
    poll: buildCodPoll(data, dayKey, viewerUserId),
    resetsDaily: true,
    communityAgreement: {
...
export async function castCodVote(userId, vote) {
  if (!["hold", "sell"].includes(vote)) throw new Error("Invalid vote");

  const data = await loadSocial();
  const dayKey = getDayKey();
  const existing = (data.codVotes || []).find((v) => v.userId === userId && v.dayKey === dayKey);
  if (existing) {
    return { ok: true, poll: buildCodPoll(data, dayKey, userId), alreadyVoted: true };
  }

  await saveSocial((store) => {
    if (!store.codVotes) store.codVotes = [];
    store.codVotes.push({
      userId,
      dayKey,
      vote,
  ...
  const refreshed = await loadSocial();
  return { ok: true, poll: buildCodPoll(refreshed, dayKey, userId), alreadyVoted: false };
}
