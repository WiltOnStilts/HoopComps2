...
import { migrateScanTracking, mergeCollectionByFingerprint } from "./card-fingerprint.mjs";
...
export async function pgMigrateAllUserScanTracking() {
  ...
  for (const row of rows) {
    ...
    if (version < 2 || before !== after || state.scoutCount !== Object.keys(state.scannedCards || {}).length) {
      await pool.query(
