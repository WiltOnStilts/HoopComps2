...
import { findUserByEmail, findUserById, createUser, getUserState, saveUserState, updatePasswordHash } from "./db.mjs";
...
export function resetPassword({ email, newPassword, displayName }) {
  const normalized = email?.toLowerCase()?.trim();
  if (!normalized || !normalized.includes("@")) {
    throw new Error("Valid email required");
  }
  ...
  const row = findUserByEmail(normalized);
  if (row) {
    updatePasswordHash(row.id, hashPassword(newPassword));
    return loginUser({ email: normalized, password: newPassword });
  }
  // Server data can reset on redeploy (free hosting) — recreate the account.
  return registerUser({
    email: normalized,
...
export function mergeGuestIntoCloud(guestState, cloudState) {
  // Cloud first, then guest — device profile choices (like leaderboard opt-in) win.
  const mergedProfile = {
    ...(cloudState?.profile || {}),
    ...(guestState?.profile || {}),
  };
