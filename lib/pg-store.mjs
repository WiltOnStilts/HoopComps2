...
import { normalizePhone, normalizeUsername, displayNameKey } from "./user-lookup.mjs";
...
import { defaultAvatarSelection, defaultOwnedAvatarParts } from "./avatar-catalog.mjs";
...
export async function pgInit(connectionString) {
  ...
  await pool.query(pgPush.PUSH_SCHEMA);
}
...
export async function pgSearchUsersByFriendQuery(query, { excludeUserId, limit = 12 } = {}) {
  const { parseFriendSearchQuery } = await import("./user-lookup.mjs");
  const parsed = parseFriendSearchQuery(query);
  if (!parsed) return [];
  ...
  if (parsed.mode === "phone") {
    const { rows } = await pool.query(
      `SELECT id, email, display_name, username, phone FROM users
       ...
       LIMIT $3`,
      [`${parsed.term}%`, excludeUserId || null, limit]
    );
    return rows;
  }
  ...
  const { rows } = await pool.query(
    `SELECT id, email, display_name, username, phone FROM users
     WHERE ($2::text IS NULL OR id::text <> $2::text)
       AND (
         lower(username) LIKE $1
         OR lower(display_name) LIKE $1
         OR lower(replace(display_name, ' ', '')) LIKE $1
         OR lower(split_part(email, '@', 1)) LIKE $1
       )
     ORDER BY display_name ASC
     LIMIT $3`,
    [like, excludeUserId || null, limit]
  );
  return rows;
}

export async function pgAssertDisplayNameAvailable(userId, displayName) {
  const key = displayNameKey(displayName);
  if (!key) return;
  const { rows } = await pool.query(
    `SELECT id FROM users
     WHERE ($1::text IS NULL OR id::text <> $1::text)
       AND lower(trim(regexp_replace(display_name, '\\s+', ' ', 'g'))) = $2
     LIMIT 1`,
    [userId || null, key]
  );
  if (rows.length) throw new Error("Display name already taken");
}

async function assertUniqueContactFields(userId, { displayName, username, phone }) {
  await pgAssertDisplayNameAvailable(userId, displayName);
...
export async function pgCreateUser({ id, email, passwordHash, displayName }) {
  await pgAssertDisplayNameAvailable(null, displayName || "Scout");
...
export async function pgMigrateAllUserCollectionQuantities() {
  if (!pool) return 0;
  const { rows } = await pool.query("SELECT user_id, state_json FROM user_state");
  let updated = 0;
  for (const row of rows) {
    let state;
    try {
    ...
    if (version < 1) {
      await pool.query(`UPDATE user_state SET state_json = $1, updated_at = NOW() WHERE user_id = $2`, [
...
export async function pgMigrateAllUserScanTracking() {
  if (!pool) return 0;
  const { rows } = await pool.query("SELECT user_id, state_json, public_leaderboard FROM user_state");
  let updated = 0;
  for (const row of rows) {
    let state;
    try {
    ...
    if (version < 3 || before !== after || state.scoutCount !== Object.keys(state.scannedCards || {}).length) {
      await pool.query(
        `UPDATE user_state SET state_json = $1, updated_at = NOW() WHERE user_id = $2`,
        [JSON.stringify(state), row.user_id]
      );
      updated += 1;
    }
  }
...
export async function pgListUserStates() {
  if (!pool) return [];
  const { rows } = await pool.query("SELECT user_id, state_json FROM user_state");
  return rows;
}

export async function pgMigrateAllUserEconomy() {
  if (!pool) return 0;
  const { rows } = await pool.query("SELECT user_id, state_json FROM user_state");
  let updated = 0;
  for (const row of rows) {
